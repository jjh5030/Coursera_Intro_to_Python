...
asteroid_info = ImageInfo([45, 45], [90, 90], 40, False)
...
def group_collide(obj_set, other_object):
    global explosion_group
    remove_set = set()
    for item in obj_set:
        if item.collide(other_object):
            remove_set.add(item)
            boom_baby = Sprite(item.get_position(), [0, 0], 0, 0, explosion_image, explosion_info, explosion_sound)
            explosion_group.add(boom_baby)
    if len(remove_set) > 0:
        obj_set.difference_update(remove_set)
        return True
    else:
...
class Sprite:
    ...
    def draw(self, canvas):
        global time, EXPLOSION_CENTER, EXPLOSION_SIZE, EXPLOSION_DIM
        if self.animated:
            explosion_index = [self.age % EXPLOSION_DIM[0], 0]
...
def draw(canvas):
    global time, started, lives, rock_group, my_ship, score, explosion_group
